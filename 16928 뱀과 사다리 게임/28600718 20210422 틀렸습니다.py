import sys

input = sys.stdin.readline
n, m = map(int, input().split())
ladder_snake = {}
snake_ladder = {}
for i in range(n + m):
    key, value = map(int, input().split())
    ladder_snake[key] = value
    snake_ladder[value] = key
climb_from_start = set(ladder_snake.keys())
climb_from_end = set(ladder_snake.values())
start = {1}
end = {100}
count = 0
while True:
    count += 1
    from_start = set()
    for i in start:
        for j in range(1, 7):
            from_start.add(i + j)
    common = from_start & climb_from_start
    candidate_from_start = from_start
    for i in common:
        candidate_from_start.add(ladder_snake[i])
    if candidate_from_start & end:
        break
    start = candidate_from_start - common
    # 여기서 사다리나 뱀을 타야 하는 숫자는 더이상 넓혀주면 안된다.

    count += 1
    from_end = set()
    for i in end:
        for j in range(1, 7):
            from_end.add(i - j)
    common = from_end & climb_from_end
    candidate_from_end = from_end
    for i in common:
        candidate_from_end.add(snake_ladder[i])
    if candidate_from_end & start:
        break
    end = candidate_from_end - common
print(count)
