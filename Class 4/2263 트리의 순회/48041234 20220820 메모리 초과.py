...
def make_tree(root_node, inorder, postorder_dict):
    if len(inorder) <= 1:
        return
    target_index = inorder.index(root_node.num)
    left_inorder = inorder[:target_index]
    if len(left_inorder) > 1:
        ...
        make_tree(root_node.left, left_inorder, postorder_dict)
    elif len(left_inorder) == 1:
        root_node.left = Node(left_inorder[0])

    right_inorder = inorder[target_index + 1:]
    if len(right_inorder) > 1:
        ...
        make_tree(root_node.right, right_inorder, postorder_dict)
...
make_tree(root_node, inorder, postorder_dict)
lst = []
preorder(root_node)
for i in lst[:-1]:
    print(i, end=' ')
